...
在固定 `p=30,q=25,cov-scale=0.2,eta=1` 的设置下，进一步运行了样本量 sweep：

```text
n = 300, 400, 500, 700, 1000, 1500, 2000
```

所有点都按照 `window=100, tol=3e-6, frob_tol=1e-3` 的规则收敛。最终 Frobenius error 随样本量整体下降：

```text
n=300:  ||M_hat-M0||_F = 5.899
n=500:  ||M_hat-M0||_F = 3.911
n=1000: ||M_hat-M0||_F = 2.334
n=2000: ||M_hat-M0||_F = 1.716
```

对 log-log 曲线做线性拟合得到斜率 `-0.688`。由于 `1/sqrt(n)` 对应斜率为 `-0.5`，这组单 seed 结果的下降趋势不慢于 `1/sqrt(n)`。不过 `n=300/400` 以及 `n=1500/2000` 附近仍有随机波动；若要作为正式 rate 证据，应对多个 seed 重复并报告均值和标准误。
